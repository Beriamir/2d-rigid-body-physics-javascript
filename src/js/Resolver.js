...
export function separatesBodies(bodyA, bodyB, normal, overlapDepth) {
  if (bodyA.isStatic && !bodyB.isStatic) {
    bodyB.move(normal, overlapDepth * 2);
  } else if (!bodyA.isStatic && bodyB.isStatic) {
    bodyA.move(normal, -overlapDepth * 2);
  } else if (!bodyA.isStatic && !bodyB.isStatic) {
    bodyA.move(normal, -overlapDepth);
    bodyB.move(normal, overlapDepth);
  }
}

export function resolveCollision(bodyA, bodyB, normal, contactPoints) {
  let raList = [];
  let rbList = [];
  ...
  const dampFactor = 0.999;
  const torqueFactor = 0.4;

  for (let i = 0; i < contactPoints.length; i++) {
    const ra = Vector2.subtract(
      contactPoints[i],
      bodyA.position || bodyA.getCentroid()
    );
    const rb = Vector2.subtract(
      contactPoints[i],
      bodyB.position || bodyB.getCentroid()
    );
  ...
  for (let i = 0; i < contactPoints.length; i++) {
    ...
    const frictionTorqueA = ra.cross(Vector2.scale(tangent, -frictionImpulse));
